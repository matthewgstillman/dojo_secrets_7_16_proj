...
class SecretManager(models.Manager):
    def validate(self, postedSecret, userid):
        ...
        if len(postedSecret) < 4:
            return(False, "Secrets must be at least four characters long, bruh!")
        try:
            currentuser = User.objects.get(id=userid)
            self.create(secret=postedSecret, author=currentuser)
            return(True, "Your secret is safe with us, bruh!")
        except:
            return(False, "We could not create this secret at this time")

    def newlike(self, secretid, userid):
        #Determin that correct secret is being liked
        try:
            secret = self.get(id=secretid)
        except:
            return(False, "This secret is not found in our database, bruh!")
        user = User.objects.get(id=userid)
        #Protect against vane people liking their own secrets
        if secret.author == user:
            return(False, "Shame on you, you can't, and shouldn't like your own secrets, bruh!")
        # Add liker(s)
        secret.likers.add(user)
        return(True, "You liked this, bruh!")

    def deletelike(self, secretid, userid):
        #Determine that correct secret is being deleted
        try:
            secret = self.get(id=secretid)
        except:
            return(False, "This secret is not found in our database, bruh!")
        user = User.objects.get(id=userid)
        if secret.author != user:
            return(False, "Shame on you, fucker! You can't delete other people's secrets on this website, bruh!")
        #If checks pass, delete secret
        secret.delete()
        return(True, "Secret Deleted, bruh")
